# %% [markdown]
# # 08 — Gradient Boosting Grid Search
#
# Train and tune a Gradient Boosting regression model using:
#
# - An untouched final test set
# - 5-fold cross-validation
# - GridSearchCV
# - Multiple Gradient Boosting hyperparameters
#
# Workflow:
#
#     Complete Dataset
...
#
# Gradient Boosting begins with a simple initial prediction.
#
# It then builds small Decision Trees sequentially:
#
#     Initial Prediction
#            ↓
#     Tree 1 Predicts Corrections
#            ↓
#     Update Predictions
#            ↓
#     Tree 2 Predicts Remaining Corrections
#            ↓
#     Continue
#
# The central idea:
#
#     Each tree predicts corrections, not house prices directly.
...
# `n_estimators`
#
#     Number of sequential correction trees.
#
# `learning_rate`
#
#     Controls how much of each tree's correction is added.
#
#     New prediction
#         =
#     Current prediction
#         +
#     Learning rate × Tree correction
#
# `max_depth`
#
#     Maximum depth of each individual correction tree.
#
#     Gradient Boosting generally uses small trees rather than the
#     deep trees commonly found inside a Random Forest.
#
# `min_samples_leaf`
#
#     Minimum number of training observations allowed in a leaf.
#
# Because the estimator is inside a Pipeline, each parameter follows:
#
...
PARAMETER_GRID: dict[str, list[Any]] = {
    ...
    "gradient_boosting__min_samples_leaf": [
        5,
        15,
    ],
}
...
def validate_columns(dataframe: pl.DataFrame) -> None:
    """Confirm that all selected features and the target are present."""
...
#
# The model starts with squared-error loss.
#
# For squared-error regression:
#
# - The initial prediction is based on the average target.
# - Each new tree learns the remaining residuals.
# - The tree's predicted corrections are multiplied by learning_rate.
# - The corrections are added to the current predictions.


# %%
def build_pipeline() -> Pipeline:
    """Create an untrained Gradient Boosting regression pipeline."""
...
# `scoring="r2"`
#
#     Select the model with the highest mean validation R².
#
# `refit=True`
#
#     After finding the winning settings, train that model again using
#     the complete training dataset.
#
# `return_train_score=True`
#
#     Save training-fold scores as well as validation-fold scores.
#
# `n_jobs=-1`
#
#     Use all available processor cores for the grid search.
...
def create_grid_search(
    ...
    """Create the Gradient Boosting hyperparameter search."""
    return GridSearchCV(
        estimator=pipeline,
...
def print_best_search_result(
    ...
    """Print the winning settings and mean validation score."""
    ...
    print(
        f"Number of correction trees: "
        f"{best_model.n_estimators_:,}"
    )
...
def print_final_model_results(
    ...
    """Print training, cross-validation, and test performance."""
...
# %% [markdown]
# ## Show how predictions improve as trees are added
#
# `staged_predict()` returns the model's predictions after:
#
# - Tree 1
# - Tree 2
# - Tree 3
# - ...
#
# This lets us observe Gradient Boosting's sequential learning process.
#
# We report performance at several stages rather than printing all
# hundreds of stages.


# %%
def create_staged_performance_table(
    best_pipeline: Pipeline,
    X_test: np.ndarray,
    y_test: np.ndarray,
) -> pl.DataFrame:
    """Show test performance as correction trees are added."""
    model = best_pipeline.named_steps[
        "gradient_boosting"
    ]
    ...
    for tree_number, predictions in enumerate(
        model.staged_predict(X_test),
        start=1,
    ):
        if tree_number not in stages_to_report:
            continue

        rows.append(
            {
                "trees_added": tree_number,
                "mae": float(
                    mean_absolute_error(
                        y_test,
                        predictions,
...
#
# Gradient Boosting importance summarizes how useful each feature was
# across all sequential correction trees.
#
# Importance does not show whether a feature raises or lowers price.
...
def create_feature_importance_table(
    ...
    """Create feature importances from the fitted boosting model."""
    model = best_pipeline.named_steps[
        "gradient_boosting"
    ]
...
# %%
def main() -> None:
    """Run the Gradient Boosting GridSearchCV workflow."""
    ...
    #
    # GridSearchCV uses only X_train and y_train.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 6. Build an unfitted Gradient Boosting pipeline.
    #
    # GridSearchCV clones this pipeline for every candidate and fold.
    # ------------------------------------------------------------------
    ...
    # This combines:
    #
    # - The Gradient Boosting pipeline
    # - The hyperparameter grid
    # - Five-fold cross-validation
    # - R² as the selection metric
    # ------------------------------------------------------------------
    ...
    #
    # Gradient Boosting trees are sequential inside each candidate:
    # Tree 2 cannot be built until Tree 1 has produced its corrections.
    #
    # GridSearchCV can still evaluate separate candidates and folds
    # in parallel.
    # ------------------------------------------------------------------
    grid_search.fit(
        X_train,
    ...
    #
    # GridSearchCV has already refitted it using all training data.
    # ------------------------------------------------------------------
    best_pipeline = grid_search.best_estimator_

    # ------------------------------------------------------------------
    # 14. Generate predictions for the training and final test sets.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 16. Evaluate the model on the untouched final test set.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 17. Display training, cross-validation, and test performance.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 18. Show how test performance changes as correction trees are added.
    #
    # This is the section that most directly demonstrates boosting:
    # each successive tree modifies the existing model.
    # ------------------------------------------------------------------
    staged_performance = create_staged_performance_table(
        best_pipeline=best_pipeline,
        X_test=X_test,
        y_test=y_test,
    )

    print("\nPerformance as correction trees are added")
    print("-" * 60)
    print(staged_performance)

    # ------------------------------------------------------------------
    # 19. Inspect several individual final predictions.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 20. Examine feature importance across all correction trees.
    # ------------------------------------------------------------------
    feature_importance_table = (
        create_feature_importance_table(best_pipeline)
    )

    print("\nBest Gradient Boosting feature importance")
    print("-" * 60)
